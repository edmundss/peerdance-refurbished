	...
	<div class="col-lg-4">
		<div class="card card-solid">
			<header class="card-heading">
				<h1 class="card-title">About Combo</h1>
				<ul class="card-actions icons right-top">
                    <li>
                      <a href="{{route('combinations.edit', $combination->id)}}">
				...
				<table class="table no-margin">
					...
					<tr>
						<th>Dance</th>
						<td><a href="{{route('dances.show', $combination->dance_id)}}">{{$combination->dance->title}}</a></td>
					</tr>
					<tr>
						<th>Author</th>
						<td>{{$combination->author ?? "Unknown"}}</td>
					</tr>
					<tr>
			...
			<div class="card-body" id='description'>
				<p>
					{!!(strlen($combination->description) >0)?$combination->description:'No description'!!}
				</p>
			</div>
			<table class="table no-margin">
				<tr>
					<th class="col-lg-3" ="col-lg-2"><nobr>Step transcript <button class="btn btn-xs btn-primary m-0 p-0" data-toggle="modal" data-target="#component-form"><i class="zmdi zmdi-plus"></i></button></nobr></th>
					<td>
						<ol>
							@forelse($combination->components as $c)
								<li>@if($c->step_id)<a href="{{route('step.show', $c->step_id)}}">{{$c->step->title}}</a>@if($c->description) - @endif @endif{{$c->description}}</li>
							@empty
								<p>There is no step transcript for this combo</p>
							@endforelse
						</ol>
					</td>
				</tr>
		...
		<div class="card card-heading-right video-list-card">
          <header class="card-heading md-bg-grey-900 p-0">
            ...
            <ul class="nav nav-pills nav-stacked">
            	@forelse($combination->videos as $v)
			...
			@endif
              ...
              <table class="table info-table" style="margin-top: -59px">
				...
				<tr>
					...
					<td><a href="{{ route('user.show', $video->creator) }}">{{$video->creator->name}}</a>, {{date('d-m-Y', strtotime($video->created_at))}}</td>
				</tr>
				@endif
				@if($video->editor)
				<tr>
					<th class="col-lg-3">Last updated</th>
					<td><a href="{{ route('user.show', $video->editor) }}">{{$video->editor->name}}</a>, {{date('d-m-Y', strtotime($video->updated_at))}}</td>
				</tr>
				@endif
              	<tr>
              		<th>Timeframe</th>
...
@section('scripts')
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#step_id").select2({
			    placeholder: "Norādi soli, ja tas ir katalogā",
			    ajax: {
				    dataType: 'json',
				    url: "{{route('steps.select2')}}?dance_id={{$combination->dance_id}}",
				    delay: 250,
				    selectOnBlur: true,
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Add transcript</h4>
          </div>
            {{Form::open(array('url'=>route('components.store'), 'method' => 'POST'))}}
            {{Form::hidden('parent_id', $combination->id)}}
            {{Form::hidden('parent_class', 'Combination')}}
          <div class="modal-body">
            <div class="form-group">
              {{Form::label('step_id', 'Select step from library')}}
              {{Form::select('step_id', [], null, ['class' => 'form-control', 'id'=>'step_id'])}}
            </div>
            <div class="text-center">
            	-- AND/OR --
            </div>
            <div class="form-group">
              {{Form::label('description', 'Free form description')}}
