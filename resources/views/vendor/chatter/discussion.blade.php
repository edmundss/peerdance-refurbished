...
<div id="chatter" class="discussion">
	...
	<div id="chatter_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			...
			<h1>{{ $discussion->title }}</h1><span class="chatter_head_details"> {{__('Posted in')}} <a class="chatter_cat" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
            ...
            @endif
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
						...
						<div id="discussion_response_email">
							<button id="submit_response" class="btn btn-success pull-right"><i class="chatter-new"></i> Submit</button>
				...
				@else
					...
					<div id="login_or_register">
						<p>
                            @lang('Please login or register to leave a response', ['home' => Config::get('chatter.routes.home')])
                        </p>
					</div>
...
<script>
	$('document').ready(function(){
		...
		$('.chatter_edit_btn').click(function(){
			...
			container.append('<div class="chatter_update_actions"><button class="btn btn-success pull-right update_chatter_edit"  data-id="' + id + '" data-markdown="' + markdown + '">Update</button><button href="/" class="btn btn-default pull-right cancel_chatter_edit" data-id="' + id + '"  data-markdown="' + markdown + '">Cancel</button></div>');
